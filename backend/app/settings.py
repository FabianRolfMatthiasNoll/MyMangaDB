...
def get_image_path():
    ...
    temp_engine = create_engine("sqlite:///./MyMangaDB.db", connect_args={"check_same_thread": False})
    SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=temp_engine)
    db = SessionLocal()
    
    try:
        # Try to get image path from settings
        result = db.execute(text("SELECT value FROM settings WHERE key = 'image_path'")).first()
        if result:
            return result[0]
    except:
        pass
