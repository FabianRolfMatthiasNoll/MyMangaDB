...
async def export_database() -> Response:
    ...
    try:
            ...
            if os.path.exists(IMAGE_PATH):
                ...
                for root, _, files in os.walk(IMAGE_PATH):
                    for file in files:
                        ...
                        arcname = os.path.relpath(file_path, os.path.dirname(IMAGE_PATH))
                        logging.info(f"Adding file to ZIP: {arcname}")
                        try:
                            zipf.write(file_path, arcname)
                        except Exception as e:
                            logging.error(f"Error adding file {file_path} to ZIP: {str(e)}")
                            continue
        ...
        return FileResponse(
            ...
            headers={
                'Content-Disposition': f'attachment; filename="mangadb_export.zip"',
                'Content-Type': 'application/zip',
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(temp_zip, 'r') as zipf:
                ...
                if not db_file:
                    raise HTTPException(status_code=400, detail="No database file found in the ZIP")
                ...
                if os.path.exists(DATABASE_PATH):
                    shutil.copy2(DATABASE_PATH, os.path.join(backup_dir, "database_backup.db"))
